...
def main() -> None:
    ...
    if args.frame is not None:
        target_filename = f"frame_{args.frame:03d}.jpg"

        frames = [
            frame
            for frame in frames
            if frame.get("file") == target_filename
        ]
    ...
    for position, frame_data in enumerate(frames, start=1):
        ...
        print(
            f"[{position:03d}/{total_frames:03d}] "
        ...
        try:
            analysis, processing_time = describe_frame(
            ...
            if analysis["scene"]:
                text_parts.append(
                    f"Escena: {analysis['scene']}."
                )

            if analysis["setting"]:
                text_parts.append(
                    f"Entorno: {analysis['setting']}."
                )

            if people_text:
                text_parts.append(
                    f"Personas y roles: {', '.join(people_text)}."
                )

            if analysis["objects"]:
                text_parts.append(
                    f"Objetos: {', '.join(analysis['objects'])}."
                )

            if analysis["actions"]:
                text_parts.append(
                    f"Acciones: {', '.join(analysis['actions'])}."
                )

            if analysis["visible_text"]:
                text_parts.append(
                    f"Texto visible: {', '.join(analysis['visible_text'])}."
                )

            if analysis["organizations"]:
                text_parts.append(
                    f"Organizaciones y marcas: "
                    f"{', '.join(analysis['organizations'])}."
                )

            if analysis["topics"]:
                text_parts.append(
                    f"Temas: {', '.join(analysis['topics'])}."
                )

            if analysis["visual_elements"]:
                text_parts.append(
                    f"Elementos visuales: "
                    f"{', '.join(analysis['visual_elements'])}."
                )

            if analysis["description"]:
                text_parts.append(
                    f"Descripción: {analysis['description']}"
                )
            ...
            if not index_text:
                raise ValueError(
                    "El modelo devolvió un análisis vacío."
                )
            ...
            # Guardado incremental para no perder el trabajo si se interrumpe.
    ...
    if processing_times:
        ...
        minimum_time = min(processing_times)
    ...
    frames_per_second = (
        successful_frames / total_time if total_time > 0 else 0.0
    )
